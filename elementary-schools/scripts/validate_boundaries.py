"""Validate elementary-school GeoJSON snapshots and explicit jurisdiction-aware crosswalk."""
...
def validate_boundaries(project_root: Path) -> BoundaryValidation:
    ...
    school_jurisdictions = {school["id"]: school["jurisdiction"] for school in schools}
    school_types = {school["id"]: school["school_type"] for school in schools}
    ...
    for entry in manifest:
        ...
        layer_linked_school_ids: set[str] = set()
        for source_name in source_names:
            ...
            if status in {"matched", "matched_alias", "matched_context"}:
                matched_features += 1
                target_ids = mapping.get("school_ids") or ([school_id] if school_id else [])
                if not target_ids:
                    result.errors.append(f"{label}:{source_name} matched record has no school target")
                for target_id in target_ids:
                    if target_id not in school_ids:
                        result.errors.append(f"{label}:{source_name} points to unknown school {target_id!r}")
                    else:
                        linked_school_ids.add(target_id)
                        layer_linked_school_ids.add(target_id)
                        if school_types.get(target_id) == "charter":
                            result.errors.append(f"{label}:{source_name} assigns a polygon to charter {target_id!r}")
            elif status == "unmatched":
                unmatched_features += 1
                if school_id is not None or mapping.get("school_ids") or not mapping.get("reason"):
        ...
        if entry.get("boundary_type") == "attendance_boundary":
            wrong_jurisdiction = [
                school_id for school_id in layer_linked_school_ids
                if school_jurisdictions.get(school_id) != entry.get("jurisdiction")
            ]
            if wrong_jurisdiction:
                result.errors.append(f"{label} cross-jurisdiction school links: {sorted(wrong_jurisdiction)}")
