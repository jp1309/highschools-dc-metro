(() => {
  ...
  const DATA_VERSION = "2026-08-30-user-ratings";
  const DATA_URLS = {
  ...
  const state = {
    ...
    filters: { query: "", jurisdiction: "", minRating: 0 },
    map: null,
    pointGroup: null,
    boundaryGroup: null,
  ...
  function populateJurisdictions() {
    const values = [...new Set(state.schools.map((school) => school.jurisdiction))].sort((a, b) => a.localeCompare(b, "es"));
    for (const value of values) {
      const option = document.createElement("option");
      option.value = value;
      option.textContent = value;
      elements.jurisdiction.append(option);
    }
  }
  ...
  function schoolPopup(school) {
    ...
    return `
      ...
      <p class="popup-source">${source}<br><strong>Estado:</strong> ${escapeHtml(ratingStatusLabel(school.rating_status))}<br><strong>Vigencia de la fuente:</strong> ${escapeHtml(effectiveDate)}<br><strong>Consultada:</strong> ${escapeHtml(checkedDate)}</p>`;
  }

  function addSchoolPoints() {
    for (const school of state.schools) {
      if (!Number.isFinite(school.lat) || !Number.isFinite(school.lng) || Math.abs(school.lat) > 90 || Math.abs(school.lng) > 180) continue;
      const category = ratingCategory(school.rating);
  ...
  function normalizeCrosswalk(raw) {
    ...
    const add = (file, featureName, schoolId) => {
      if (!file || featureName === undefined || featureName === null) return;
      const value = schoolId && schoolId !== "unmatched" ? String(schoolId) : null;
    ...
    const addRows = (rows) => {
      for (const row of rows) {
        ...
        if (Array.isArray(row.mappings)) {
          for (const mapping of row.mappings) add(file, mapping.feature_name ?? mapping.name ?? mapping.boundary_name, mapping.school_id);
        } else if (row.mappings && typeof row.mappings === "object") {
          for (const [featureName, schoolId] of Object.entries(row.mappings)) {
            add(file, featureName, typeof schoolId === "object" ? schoolId?.school_id : schoolId);
          }
        } else {
          add(file, row.feature_name ?? row.name ?? row.boundary_name, row.school_id);
        }
      }
    };
    const addObject = (object) => {
      for (const [file, mappings] of Object.entries(object || {})) {
        if (Array.isArray(mappings)) {
          for (const row of mappings) add(file, row.feature_name ?? row.name ?? row.boundary_name, row.school_id);
        } else if (mappings && typeof mappings === "object") {
          for (const [featureName, schoolId] of Object.entries(mappings)) {
            add(file, featureName, typeof schoolId === "object" ? schoolId?.school_id : schoolId);
          }
        }
      }
  ...
  function lookupSchoolId(crosswalk, entry, featureName) {
  ...
  function boundaryPopup(entry, featureName, school) {
    ...
    const title = school?.name || featureName || "Límite sin nombre";
    const relation = school ? `Vinculado por ID: ${school.id}` : "Sin escuela vinculada";
    return `
      <div class="popup-title">${escapeHtml(title)}</div>
      <p class="popup-meta"><strong>Nombre en la fuente:</strong> ${escapeHtml(featureName || "No indicado")}</p>
      <p class="popup-meta"><strong>Jurisdicción:</strong> ${escapeHtml(entry.jurisdiction || school?.jurisdiction || "No indicada")}</p>
  ...
  async function loadBoundaryEntry(entry, crosswalk) {
    ...
    for (const feature of geojson.features) {
      ...
      const schoolId = lookupSchoolId(crosswalk, entry, featureName);
      const school = schoolId ? state.schoolsById.get(schoolId) : null;
      ...
      const geoLayer = L.geoJSON(feature, {
        ...
        style: boundaryStyle(school, entry.boundary_type),
        onEachFeature: (_item, layer) => {
          layer.bindPopup(boundaryPopup(entry, featureName, school));
          layer.on({
            mouseover: () => layer.setStyle({
              weight: 3.4,
              fillOpacity: attendanceBoundary ? 0.24 : 0.32
            }),
            mouseout: () => layer.setStyle(boundaryStyle(school, entry.boundary_type))
          });
          ...
          state.boundaryRecords.push({
            ...
            jurisdiction: String(entry.jurisdiction || school?.jurisdiction || ""),
            boundaryType: entry.boundary_type
          });
          if (school && attendanceBoundary) state.linkedSchoolIds.add(school.id);
          if (school && !attendanceBoundary) state.contextualSchoolIds.add(school.id);
        }
      });
  ...
  function schoolMatches(school) {
    ...
    const haystack = normalizeForSearch(`${school.name} ${school.address} ${school.jurisdiction}`);
  ...
  function boundaryMatches(record) {
    if (record.school) return schoolMatches(record.school);
  ...
  function renderList(visibleSchools) {
    ...
    for (const school of visibleSchools) {
      ...
      button.setAttribute("aria-label", `${school.name}, ${school.jurisdiction}, calificación ${ratingLabel(school.rating)}. Mostrar en el mapa.`);
      button.innerHTML = `
        <span class="school-name">${escapeHtml(school.name)}</span>
        <span class="school-meta">${escapeHtml(school.jurisdiction)} · ${escapeHtml(school.address)}</span>
        <span class="rating-badge rating-${category}">${escapeHtml(ratingLabel(school.rating))}</span>`;
  ...
  function renderSummary() {
    const verifiedSchools = state.schools.filter((school) => school.rating_status === "verified");
    ...
    elements.coverage.textContent = `${state.schools.length} escuelas únicas en ${jurisdictions} jurisdicciones; ${state.linkedSchoolIds.size} con zona oficial, ${state.contextualSchoolIds.size} con contexto municipal y ${verifiedSchools.length} con calificación verificada.`;
  }
  ...
  function bindControls() {
    ...
    elements.filters.addEventListener("reset", () => {
      window.setTimeout(() => {
        state.filters = { query: "", jurisdiction: "", minRating: 0 };
        updateView();
  ...
  async function start() {
    try {
      initMap();
      bindControls();
      const [schoolsResult, manifestResult, crosswalkResult] = await Promise.allSettled([
      ...
      const unverifiedRatings = state.schools.filter((school) => school.rating_status !== "verified" && school.rating_status !== "not_available").length;
