...
RUN_ID = "locations-official-20260912"
...
def _explicit_address_feature(
    alias: dict[str, object] | None,
    features_by_source: dict[str, list[dict[str, object]]],
) -> dict[str, object] | None:
    """Resolve an explicitly documented address record, including extraterritorial campuses."""
    if not alias or not alias.get("address_source_id"):
        return None
    source_id = str(alias["address_source_id"])
    record_id = str(alias.get("address_source_record_id") or "")
    candidates = [
        feature for feature in features_by_source.get(source_id, [])
        if feature["source_record_id"] == record_id and feature.get("address")
    ]
    if len(candidates) != 1:
        raise RefreshFailure(f"Explicit address record for {alias.get('school_id')} resolved to {len(candidates)} records")
    return candidates[0]
...
def refresh(project_root: Path) -> dict[str, int]:
    ...
    used_records: dict[tuple[str, str], str] = {}
    for school in schools:
        ...
        if record_key in used_records:
            raise RefreshFailure(
                f"Official source record reused by multiple schools: {record_key}; "
                f"{used_records[record_key]} and {school['id']}"
            )
        used_records[record_key] = str(school["id"])
        address_feature = _explicit_address_feature(alias, features_by_source) or _supplement_address(
            school, feature, source_by_id, features_by_source
        )
